...
import           Data.Forest.Static.AlignPermuteRL
...
--runIO f1 f2 matchSc mismatchSc indelSc temperature = (fwd,unId $ axiom f)
...
runAlignS t1' t2' matchSc notmatchSc delinSc = do
  ...
  print itt
  print ""
  let (lll,uuu) = bounds imt
  print lll
  print uuu
  print ""
  mapM_ (\(i,s) -> print (i, linearIndex lll uuu $ i, s)) $ assocs ift
  print ""
  mapM_ (\(i,s) -> print (i, linearIndex lll uuu $ i, s)) $ assocs imt
  print ""
  mapM_ (\(i,s) -> print (i, linearIndex lll uuu $ i, s)) $ assocs itt
  let us = sort . map (linearIndex lll uuu . fst) $ assocs ift
  let vs = nub us
  print $ us \\ vs
  printf "Score: %10d\n" sc
--  forM_ bt $ \b -> do
...
--      t1 = f $ Text.pack t1'
--      t2 = f $ Text.pack t2'
--  let (inn,_) = runIO t1 t2 matchSc mismatchSc indelSc temperature -- (t2 {F.lsib = VG.fromList [-1,-1], F.rsib = VG.fromList [-1,-1]})
